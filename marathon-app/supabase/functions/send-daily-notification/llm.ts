/**
 * Same Gemini-primary/Groq-fallback pattern as coach-chat/llm.ts and
 * generate-run-script/llm.ts - duplicated rather than cross-imported since
 * Supabase deploys each function directory independently. Plain text this
 * time (one short line), no JSON mode needed.
 */
...
export async function generateMotivationLine(systemPrompt: string, userMessage: string): Promise<string> {
  try {
