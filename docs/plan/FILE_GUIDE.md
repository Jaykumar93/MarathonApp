...
| [activities.ts](../../marathon-app/lib/data/activities.ts) | `getActivitiesInRange`, `groupActivitiesByDate` (read), plus the Task 5 write path: `createActivity` (also marks the linked planned session completed, if any), `getActivityById`, `getAllActivities`. |
...
- **`npx tsc --noEmit` and `npm test` (56 tests) should both stay clean** after any change — this has been true throughout the project and is the fastest signal something broke.
