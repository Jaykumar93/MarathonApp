...
| [app/(tabs)/index.tsx](../../marathon-app/app/(tabs)/index.tsx) | `/` (Home) | Countdown, full-plan scrollable calendar + day detail, month activity chart, weekly mileage (real logged distance vs. the current plan week's target). Shows `NoPlanPrompt` if no active goal/plan. Carries a `LogFab`. |
| [app/(tabs)/plan.tsx](../../marathon-app/app/(tabs)/plan.tsx) | `/plan` | Block Profile (hero terrain chart — actual-progress only, see `BlockProfile.tsx`), scrollable calendar + day detail, current week's session list with move/mark-done actions. Shows `NoPlanPrompt` if no active goal/plan. Carries a `LogFab`. |
| [app/(tabs)/track.tsx](../../marathon-app/app/(tabs)/track.tsx) | `/track` | GPS-run entry point (Task 6). Shows today's planned session as context (if any), a "Start run" button into `active-run` (passing its `planSessionId` through when one exists), and a pending-sync banner when `pendingActivities` has anything offline-queued. |
| [app/(tabs)/activity.tsx](../../marathon-app/app/(tabs)/activity.tsx) | `/activity` | Activity History (Task 5). This-week/this-month stat cards (distance paired with time under one heading), a Type dropdown, and a "Filters" button (badge shows active count) opening a bottom-sheet with date-range presets (All time/This week/Last week/This month/Last month/Custom range with `DateField` From/To) plus min-distance/min-duration thresholds. List grouped by calendar month, tap a row → `run-summary`. Carries a `LogFab`. |
...
| [app/log-activity.tsx](../../marathon-app/app/log-activity.tsx) | `/log-activity` | Also outside the tabs group. Manual Log Entry (Task 5) - date/type/distance/duration/optional RPE, plus an expandable notes/HR/elevation section. Accepts optional `planSessionId`+`date` params to pre-fill from a specific planned session (and, on save, marks that session completed). Reached from every `LogFab`. |
| [app/run-summary.tsx](../../marathon-app/app/run-summary.tsx) | `/run-summary` | Also outside the tabs group. Doubles as the post-save confirmation (landed on right after `log-activity`'s or `active-run`'s Save) and a general Activity Detail view (tapping any row in Activity History or `DayDetailPanel`'s Logged section) - same `id` param either way. Shows stats, a planned-vs-actual line when linked to a session, and RPE/notes/HR/elevation if present (checks are `!= null`, not truthy - a GPS run's `elevation_gain_meters` can legitimately be exactly `0`, which a truthy check would swallow while also rendering a stray literal "0" in React). No live-route map yet (Task 6, deferred - see its sub-plan). |
| [app/active-run.tsx](../../marathon-app/app/active-run.tsx) | `/active-run` | Also outside the tabs group, permanently dark. The GPS run screen (Task 6) - `expo-location` foreground tracking, live distance/pace/elevation computed via `lib/gpsStats.ts`, a simple Pace Band (goal pace from the linked planned session, if any). Pause/Resume/Stop; on Stop, saves via `createActivity` (falling back to `lib/data/pendingActivities.ts`'s offline queue on failure) and navigates to `run-summary`. No live map yet - needs a custom dev build `react-native-maps` requires, which doesn't exist for this project yet; route/splits are still fully captured and stored regardless. |

## Components (`marathon-app/components/`)

| File | What it renders |
|---|---|
| [BlockProfile.tsx](../../marathon-app/components/BlockProfile.tsx) | The SVG "terrain" chart of weekly volume, mini variant for Home (unchanged, calendar-time based), hero variant for Plan. Hero draws the full planned curve only as a faint dashed reference; the solid accent line/dots are built from **actual logged mileage** (`actualWeeklyVolumesKm`) and only extend through the consecutive run of weeks that have any - a point appears once that week's run is actually done, not because its date arrived (Task 5). |
...
| [DayDetailPanel.tsx](../../marathon-app/components/DayDetailPanel.tsx) | Selected day's planned session and any logged activity, each as a matching Distance/Duration-or-Time/Pace three-column card - outlined/white for planned, solid-filled for logged (same convention `PlanCalendarScroller`'s cells use), tap a logged card to open `run-summary`. Unit-aware (`lib/units.ts`). |
| [MonthActivityChart.tsx](../../marathon-app/components/MonthActivityChart.tsx) | Calendar-month bar chart of logged distance per day, prev/next month navigation. Press-and-hold a bar (`onPressIn`/`onPressOut`) to show that day's distance in a small tooltip anchored to the bar itself - disappears the instant it's released; tapping still selects the day as before. |
...
| [SessionListRow.tsx](../../marathon-app/components/SessionListRow.tsx) | One row in Plan's "this week's sessions" list — missed sessions get inline Move-to-tomorrow/Mark-done-anyway actions. Type label/color come from `lib/sessionTypes.ts`. |
| [ui/Badge.tsx](../../marathon-app/components/ui/Badge.tsx) | Small inline count pill (accent-filled circle, white text). Used by the "Filters" button and by `Dropdown`'s own `badge` prop so an active-filter indicator looks identical everywhere it appears. |
...
| [ui/DateField.tsx](../../marathon-app/components/ui/DateField.tsx) | Day/Month/Year date picker built on `ui/Dropdown.tsx`. Shows every day/month/year always; callers validate the result (e.g. `goalDate < today` for a race date, `date > today` for a logged run) explicitly rather than the field hiding invalid options. `yearsAhead`/`yearsBack`/`defaultOffsetDays` props let it serve both a future-date use (race date, the default) and a past-date one (logging a run, Activity History's custom date-range filter). |
| [ui/Dropdown.tsx](../../marathon-app/components/ui/Dropdown.tsx) | The closed-field-opens-a-modal-list pattern, extracted out of `DateField.tsx` once Activity History's Type/date-range filters needed the same thing. Generic over the option value type; a `compact` variant for a filter-pill look vs. the default form-field look; an optional `badge` count (`ui/Badge.tsx`) shown inline before the caret. **Backdrop and sheet are siblings, not parent/child** - a `TextInput` nested inside a `Pressable`'s own subtree can't receive focus on web (RNW's Pressable intercepts `pointerdown` for its own press handling before the browser's default focus behavior fires; `onStartShouldSetResponder` does not prevent this on web - it's a native-only responder API). Any new tap-outside-to-close modal in this app should follow this same sibling structure, not nest the sheet inside the backdrop `Pressable`. |
| [ui/LogFab.tsx](../../marathon-app/components/ui/LogFab.tsx) | Persistent floating "+" button, bottom-right, on Home/Plan/Activity - the single entry point into `log-activity`, replacing an earlier per-day inline link and Activity's own full-width button (Task 5). Deliberately context-free (always opens a blank log for today). |
...
| [plans.ts](../../marathon-app/lib/data/plans.ts) | `createPlanWithSessions` (batched insert of plan + all sessions), `getCurrentPlan`, `getPlanSessions`, `getPlanSessionById` (single-row lookup, used by `log-activity`/`run-summary` when only an id is on hand), `markSessionDone`, `moveSessionToTomorrow`, `supersedePlan` (soft-delete a plan without touching its goal - used by Edit Plan before inserting the regenerated one). |
| [activities.ts](../../marathon-app/lib/data/activities.ts) | `getActivitiesInRange`, `groupActivitiesByDate` (read), plus the write path: `createActivity` (also marks the linked planned session completed, if any), `getActivityById`, `getAllActivities`. `CreateActivityInput` covers both Task 5's hand-typed entries and Task 6's GPS runs (`startTimeIso`/`route`/`splits`/`elevationLossMeters` - GPS-only, optional); `source` is `"manual"` either way (see `active-run.tsx`'s note on why). |
| [pendingActivities.ts](../../marathon-app/lib/data/pendingActivities.ts) | Offline queue (Task 6) for a GPS run whose save-on-Stop failed - `AsyncStorage`-backed, not a server table. `savePendingActivity`, `getPendingActivityCount`, `flushPendingActivities` (retries each queued entry via `createActivity`, called when the Track tab regains focus). |
| [usePlanData.ts](../../marathon-app/lib/data/usePlanData.ts) | `useActivePlanData()` hook (loads goal+plan+sessions for the signed-in user) plus pure date-math helpers: `getCurrentWeekNumber`, `getWeekDateRange`, `getCurrentCalendarWeekRange` (the real Monday-Sunday week containing today, independent of plan-week numbering - used for "how much have I actually run this week" widgets, which disagree with `getCurrentWeekNumber` during the lead-in bridge), `getWeeklyVolumesKm` (planned), `getActualWeeklyVolumesKm` (logged, buckets `activities` by date against `plan.start_date` - feeds `BlockProfile`'s hero variant), `getAllPlanDays`, `getPlanProgressFraction` (0-1 elapsed fraction, used by `CountdownArc`). **Always derives "current week"/calendar range from real dates, never from `week_number`/session-row order** — a past bug (Task 4) showed row-order-based logic breaks the moment a session moves. |
...
| [activityStats.ts](../../marathon-app/lib/activityStats.ts) | `computeActivityStats()` — week/month distance *and* time, paired by period (not a week/year split - the two numbers for one period are meant to read together). Kept free of any `lib/supabase.ts` import (unlike `lib/data/*.ts`) specifically so it's unit-testable in Jest, which can't load the native `AsyncStorage` module the Supabase client needs - same reasoning as `timeFormat.ts`/`units.ts`. |
| [gpsStats.ts](../../marathon-app/lib/gpsStats.ts) | Pure GPS math (Task 6), same supabase-free-for-testability reasoning as `activityStats.ts`: `haversineDistanceMeters`, `computeRouteDistanceMeters`, `computeSplits` (per-km, interpolated at the exact boundary), `computeElevationGainLoss`, `computeAveragePaceSecondsPerKm`, `computeRecentPaceSecondsPerKm` (a 60s rolling window - what "current pace" means mid-run, not the whole-run average). Used by `active-run.tsx` and the activity-save path when a GPS run stops. |
| [sessionTypes.ts](../../marathon-app/lib/sessionTypes.ts) | `SESSION_TYPE_LABEL`/`SESSION_TYPE_COLOR` (shared by `SessionListRow`, `DayDetailPanel`, Activity History, and the log-activity type picker - previously duplicated per-file) and `ACTIVITY_TYPE_OPTIONS` (the loggable subset - no "rest"). |
...
| File | What it's for |
|---|---|
| [app.json](../../marathon-app/app.json) | Expo app manifest — name ("Stryde"), EAS project id, icon/splash paths, installed plugins (`expo-location` added in Task 6, with a plain-language permission description per PRD §7). No `react-native-maps` config plugin/Google Maps API key yet - that's part of the deferred map work, see `docs/plan/06-gps-tracking-active-run.md`. |
...
- **`npx tsc --noEmit` and `npm test` (68 tests) should both stay clean** after any change — this has been true throughout the project and is the fastest signal something broke.
- **A conditional render's outer `||`/`&&` chain must end on a boolean, not a possibly-`0` numeric field** — `0 && <X/>` renders the literal text "0" in React (0 is falsy but not one of the values React skips). Bit `run-summary.tsx` once a GPS run started legitimately saving `elevation_gain_meters: 0` where every prior (manual-entry) caller had only ever left it `null` or a positive number. Use an explicit `!= null` (or similar) check on any numeric field that can legitimately be zero.
- **React Navigation keeps every tab screen mounted even when hidden.** A `find`/coordinate click can land on a same-labeled element belonging to a different, currently-hidden tab. If a click seems to navigate somewhere unexpected, re-navigate to the intended tab, screenshot to confirm it's frontmost, then retry (see HANDOFF.md's Task 5 round for the most recent instance).
