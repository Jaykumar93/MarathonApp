...
## Follow-up (2026-09-04): structured interval workouts

Task 3 was already marked Done, but a real gap surfaced once Task 6's Active Run was actually being used against an "interval" session: `session_type: "interval"` had always been just a label - one flat `plannedDistanceMeters`/`plannedPaceSecondsPerKm`, identical in shape to an easy or tempo run. There was nowhere in the data model to say "6x600m at interval pace, 300m jog recovery between" - so Active Run's live pace guidance for an interval day was exactly as generic as for any other run, which isn't what interval training actually needs. Raised with the user via `AskUserQuestion` before touching anything (two decisions, since this is real architecture, not a UI tweak): (1) generate the structure for real in the plan engine, vs. estimate it live only in Active Run, vs. have the user type it in manually each time - user chose **plan engine generates it**; (2) what convention decides rep length - user chose **phase-based** (matching how `paceCalculator.ts` already scales pace by phase) over a single fixed default.

- **`IntervalStructure`** (new type in `types.ts`): `warmupMeters`, `reps`, `repDistanceMeters`, `repPaceSecondsPerKm`, `recoveryDistanceMeters`, `recoveryPaceSecondsPerKm`, `cooldownMeters`. Added as `intervalStructure?: IntervalStructure | null` on `PlanSessionDraft` - additive only, every other session type is completely unaffected.
- **`lib/planEngine/intervalStructure.ts`** (new, pure/tested - `intervalStructure.test.ts`): `buildIntervalStructure(targetDistanceKm, targetDurationSeconds, phase, paceZones)` decides rep length by phase (peak: 1000m: reps meant to be run harder/longer once base speed is established; build/taper: 600m - taper deliberately doesn't get its own tier, since its already-lower `weeklyVolumeKm` naturally shrinks rep count on its own), recovery distance at half the rep distance (standard interval-training convention), and warmup/cooldown minutes from the *same* `getDurationBucket` thresholds `prepRecoveryTemplates.ts` already uses for its free-text prep copy (so the numeric structure and the prep text agree, rather than silently disagreeing). `intervalStructureTotals()` sums the structure back into a real `distanceMeters`/`durationSeconds` - **this becomes the session's actual stored `plannedDistanceMeters`/`plannedDurationSeconds`**, not the rough `weeklyVolumeKm*INTERVAL_SHARE` estimate that seeded it (rep count has to be a whole number, so the two never match exactly). Confirmed via `getSessionTypesForWeek`/`sessionDistribution.ts` that this only ever needs to handle build/peak/taper - base and ultra plans never schedule an "interval" session type at all, so there's no "which phase gets which rep length" question for either.
- **`lib/intervalProgress.ts`** (new, pure/tested - 11 cases in `__tests__/intervalProgress.test.ts` covering every leg-boundary transition): `getCurrentLeg(structure, distanceCoveredMeters)` - purely a function of cumulative distance, no GPS/timing coupling, same dependency-free pattern as `gpsStats.ts`. Walks warmup -> (rep, recovery) x reps -> cooldown -> complete; a recovery leg follows every rep including the last one, matching `intervalStructureTotals`'s own totals formula (reps recoveries, not reps-1) so a run covering exactly the planned distance lands exactly on "complete" here too, not one recovery leg short.
- **`planGenerator.ts`**: interval days now call `buildIntervalStructure` with the same rough target the flat approach used to use directly, and store both the structure and its real totals on the session. `usedVolumeKm` bookkeeping (what's left over for easy days) still uses the *structured* total, so the week's numbers stay internally consistent.
- **Schema**: `plan_sessions.interval_structure jsonb`, nullable (migration `20260903190000_add_interval_structure.sql`, applied to the live project). Deliberately **not backfilled** - every interval session generated before this migration stays null and falls back to exactly the flat single-pace guidance it always had. No plan regeneration was forced on the live test account either; one existing interval session was patched directly with real generator output (computed via `generatePlan()` with the account's actual goal params, not hand-typed) purely to verify the UI end-to-end, since this session's browser sandbox denies geolocation and can't reach Active Run's actual "running" state to test live.
- **UI**: `formatIntervalStructureSummary()` (new, `lib/sessionTypes.ts`) renders "6 x 600m @ 4:40/km, 300m jog recovery" - shown in both `DayDetailPanel.tsx`'s planned-session card and `SessionListRow.tsx`'s Plan-tab list row, whenever `interval_structure` is present (silently absent otherwise, old data included). `lib/units.ts` gained `formatMeters()` for this (sub-km distances read as "600m" rather than "0.6km", unit-preference-independent since track-style reps are conventionally metric regardless of the user's mi/km display setting).
- **Active Run live guidance** (`app/active-run.tsx`) - the actual point of the feature: while running/paused, if the linked planned session has a structure, a highlighted banner replaces the old flat "Goal pace" line with e.g. "INTERVAL · REP 3 OF 6 — 220m to go @ 4:12/km" or "RECOVERY JOG — 180m to go @ 5:40/km", recomputed every render from `getCurrentLeg(structure, computeRouteDistanceMeters(rt.points))`. Every other session type (or an old interval session with no structure) falls back to exactly the prior single flat pace-band line, unchanged. Voice cues for this are explicitly deferred - text only for now, per the user's own framing ("for now a message... later voice assistance").
- Verified: `npx tsc --noEmit` and all tests clean (68 -> 85, +17 for the two new pure modules); live-verified in the Browser tool that `DayDetailPanel` renders the structure line correctly against the real patched session, and that an old null-`interval_structure` interval day still renders exactly as before (no crash, no stray line) - regression-checked, not just assumed. The Active Run banner itself could not be live-verified here (geolocation denied in this sandbox, same limitation as the rest of Task 6's GPS work) - relies on the 11-case `intervalProgress.test.ts` suite plus code review; worth confirming on-device.
